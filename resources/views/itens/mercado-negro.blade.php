...
@section('title', 'Mercado Negro — AlbionMillionaire')
...
<style>
  ...
  :root{
    ...
    --bm-red: #c0392b;
    --bm-red-dim: rgba(192,57,43,.18);
    --bm-red-glow: rgba(192,57,43,.08);
  }

  /* ── page header ─────────────────────────────────── */
  .page-head{position:relative;border-bottom:1px solid var(--line);overflow:hidden;background:linear-gradient(180deg,#1a0d0d,var(--bg))}
  .page-head::before{content:"";position:absolute;right:-80px;top:-120px;width:420px;height:420px;border-radius:50%;background:radial-gradient(circle,rgba(192,57,43,.15),transparent 65%)}
  ...
  .bm-badge{display:inline-flex;align-items:center;gap:6px;font-family:"JetBrains Mono",monospace;font-size:11px;letter-spacing:.1em;text-transform:uppercase;background:var(--bm-red-dim);color:#e57373;border:1px solid rgba(192,57,43,.4);border-radius:3px;padding:3px 10px;margin-left:12px;vertical-align:middle}
  .bm-badge svg{opacity:.8}
  .page-head .lead{margin-top:12px;color:var(--parch-dim);max-width:640px;font-weight:300;font-size:17px}

  /* ── filters panel ───────────────────────────────── */
  .filters-panel{background:linear-gradient(180deg,#160d0d,#14120a);border-bottom:1px solid var(--line-soft);padding:20px 0}
  ...
  .filter-input:focus{border-color:var(--bm-red);box-shadow:0 0 0 1px rgba(192,57,43,.15)}
  ...
  .filter-select:focus{border-color:var(--bm-red)}
  .filter-select option{background:#1b0f0f;color:var(--parch);text-transform:none;font-family:sans-serif;font-size:13px;font-weight:400;letter-spacing:0}
  ...
  /* ── bm info strip ───────────────────────────────── */
  .bm-info-strip{display:flex;align-items:center;gap:12px;background:var(--bm-red-glow);border:1px solid rgba(192,57,43,.25);border-radius:4px;padding:9px 14px;margin-top:14px;font-family:"JetBrains Mono",monospace;font-size:11px;color:var(--parch-faint);flex-wrap:wrap}
  .bm-info-strip svg{color:#e57373;flex:0 0 auto}
  .bm-info-chip{display:inline-flex;align-items:center;gap:5px;background:rgba(192,57,43,.12);border:1px solid rgba(192,57,43,.3);border-radius:3px;padding:2px 8px;color:#e57373;letter-spacing:.05em}
  ...
  /* ── table ───────────────────────────────────────── */
  .tablewrap-transport{overflow-x:auto;border:1px solid rgba(192,57,43,.2);border-radius:6px;background:linear-gradient(180deg,#1a0e0e,#14120a)}
  .tablewrap-transport table{min-width:960px;width:100%;border-collapse:collapse}
  .tablewrap-transport thead th{
    position:sticky;top:0;z-index:10;
    background:#1a0e0e;border-bottom:1px solid rgba(192,57,43,.25);
  ...
  .tablewrap-transport tbody tr:hover{background:rgba(192,57,43,.04)}
  ...
  .bm-city-name{color:#e57373 !important}
  ...
  @media(max-width:768px){
    ...
    .tcard{border:1px solid var(--line-soft);border-radius:6px;background:linear-gradient(180deg,#1a0e0e,#14120a);overflow:hidden;border-left:3px solid transparent}
...
<div class="filters-panel">
  <div class="wrap">
    ...
    <form method="GET" action="{{ route('mercado-negro.index') }}" id="filterForm">
      @if(request('sort') && request('sort') !== 'lucro_direto')
...
<div class="wrap" style="padding-top:8px;padding-bottom:8px">
  ...
  {{-- ── TABLE ────────────────────────────────────────────── --}}
  @php
    function bmSortUrl(string $key, string $currentKey, string $currentDir, $request): string {
        $dir = ($key === $currentKey) ? ($currentDir === 'asc' ? 'desc' : 'asc') : 'desc';
        return $request->fullUrlWithQuery(['sort' => $key, 'dir' => $dir, 'page' => 1]);
    }
    function bmSortArrow(string $key, string $currentKey, string $currentDir): string {
              ...
              @if($isSplit)
                <div class="th-split">
                  ...
                  <a href="{{ bmSortUrl($col['pct_key'], $sortKey, $sortDir, request()) }}" class="{{ $isPctSort ? 'th-active' : '' }}">
                    %{!! bmSortArrow($col['pct_key'], $sortKey, $sortDir) !!}
                  </a>
                </div>
              @else
          ...
          <tr class="{{ $rowClass }}">
            ...
            <td class="r">
              ...
              <div style="margin-top:3px">
                <span class="pct-badge {{ $ploBadge }}">{{ $plo > 0 ? '+' : '' }}{{ number_format($plo, 2, ',', '.') }}%</span>
              </div>
            </td>
            ...
            <td class="r">
              ...
              @endif
              <div style="margin-top:3px">
                <span class="pct-badge {{ $pldBadge }}">{{ $pld > 0 ? '+' : '' }}{{ number_format($pld, 2, ',', '.') }}%</span>
              </div>
            </td>
...
(function () {
  ...
  function applyLocale(locale) {
    ...
    ['[name="qualidade"] option',
     '[name="cidade_ordem"] option', '[name="cidade_compra"] option']
    .forEach(sel => {
