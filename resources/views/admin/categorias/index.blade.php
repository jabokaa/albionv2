...
<div class="breadcrumb">
  <span>Admin</span><span class="sep">/</span><span>Categorias</span>
</div>

<div class="card">
  <div class="card-head">
    <h2>Todas as categorias ({{ $categorias->total() }})</h2>
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
      <form method="GET" action="{{ route('admin.categorias.index') }}" class="search-bar">
        ...
        @if($busca)<a href="{{ route('admin.categorias.index') }}" class="btn btn-sm">Limpar</a>@endif
      </form>
    ...
    <table>
      ...
      <tbody>
        @forelse($categorias as $cat)
          <tr>
            <td>
              <span class="mono" style="font-size:13px">{{ $cat->nome }}</span>
            </td>
            <td>{{ $cat->portugues ?? '—' }}</td>
            <td>{{ $cat->ingles ?? '—' }}</td>
            <td>
              ...
              @else
                <span style="color:var(--parch-faint);font-size:13px">raiz</span>
              @endif
            </td>
            <td>
              <span class="badge">{{ $cat->filhos()->count() }}</span>
            </td>
            <td>
              <span class="badge">{{ $cat->itens()->count() }}</span>
            </td>
            <td>
              <div style="display:flex;gap:6px">
                <a href="{{ route('admin.categorias.edit', $cat) }}" class="btn btn-sm">Editar</a>
                <form method="POST" action="{{ route('admin.categorias.destroy', $cat) }}"
                      onsubmit="return confirm('Excluir a categoria «{{ $cat->nome }}»?')">
                  @csrf @method('DELETE')
        ...
        @empty
          <tr><td colspan="7" style="text-align:center;color:var(--parch-faint);padding:40px">Nenhuma categoria encontrada.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
